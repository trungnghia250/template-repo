...
func (u *userUseCase) GetUserByID(ctx context.Context, req dto.GetUserByIDRequest) (dto.GetUserByIDResponse, error) {
	var (
	...
	if err != nil {
		log.Errorf("GetUserByID with ID %s fail, err", req.ID, err)
		return resp, err
	}
...
func (u *userUseCase) DeleteUser(ctx context.Context, req dto.DeleteUserRequest) error {
	user, err := u.repo.GetUserByID(ctx, req.ID)
	if err != nil {
		log.Errorf("GetUserByID with ID %s fail, err", req.ID, err)
		return err
	...
	if err != nil {
		log.Errorf("DeleteUser with userID %s fail, err", req.ID, err)
		return err
	}
